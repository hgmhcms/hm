...
function requestRankData() {
    ...
    var url = '/e/extend/rank/index.php?line=10&flagdata='+ flag_data.join('-');
    T.restGet(url, {}, function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var comic_cover = comic.cover;
            var is_end = comic.status == 1 ? true : false;
            ...
            html +=
                ...
                '<div class="itemTxt">' +
                '<a class="title"' + ' href="'+data[i].titleurl+'" title="">' + comic.name + '</a>' +
            ...
            if(flag_data[0] == 1) {
                ...
                '<a class="comi" href="'+data[i].titleurl+'">'+comic['chapter_name']+'</a>' +
                '<p class="Tucao">' + comic['vote_num'] + '吐槽</p>' +
                '</div>';
            } else if (flag_data[0] == 2) {
                //订阅
                html += '<p class="scripSize">'+comic['sub_num']+'收藏</p>';
                var is_exist = false;
                ...
                if(is_exist){
                    html += '<a href="/e/member/fava/add/?classid='+comic.classid+'&id='+comic.id+'" class="scripBtn">点击收藏</a>'

                }else{
                    html += '<a href="/e/member/fava/add/?classid='+comic.classid+'&id='+comic.id+'" class="scripBtn">收藏漫画</a>'
                }
                html +='</div>';
            }
