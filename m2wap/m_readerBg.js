...
(function() {
    ...
    //点击进入上一话
    this.prevBtnAction=function(url){
        //this.requestChapter("prev");
		location.replace(url);
    }
    //点击进入下一话
    this.nextBtnAction=function(url){
...
function historyLog(historyJson){
    if($.cookie('my') != null){
        ...
        $.ajax({
            ...
            data: {uid:userId, st: 'comic', type: 2, json:historyJson},
            success: function (e) {
...
//获取吐槽数量
var comicUrl ="//interface.dmzj.com";//接口连接
function getPointTotal(){
    var requestUrl = comicUrl+'/api/viewpoint/getViewpointNum2';
    $.ajax({
        type: "get",
        url: requestUrl,
        dataType : "jsonp",
